...
export const CategoryPagination: React.FC<CategoryPaginationProps> = ({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-4 bg-white px-4 py-3 border border-stone-200 rounded-xl text-xs text-stone-600 shadow-xs">
      <div className="flex items-center gap-2">
        <span>Show</span>
        <select
          value={pageSize}
          onChange={(e) => onPageSizeChange(Number(e.target.value))}
          className="bg-stone-50 border border-stone-200 text-stone-700 rounded-md px-2 py-1 focus:outline-none focus:ring-1 focus:ring-amber-500 font-medium"
        >
        ...
        <span className="hidden sm:inline">
          ...
          <strong className="text-stone-900 font-semibold">{totalItems}</strong> categories
        </span>
      </div>
